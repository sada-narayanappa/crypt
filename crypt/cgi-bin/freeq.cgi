...
sub doget()
{
	...
	{
 		my $t1 = $resp->param("t1");
 		my $kl = $resp->param("keylen") || 3;
 		my $k  = $resp->param("key") || 3;
		$t1 =~ s/\n//g;
		$t1 =~ s/\r//g;
		$t1 =~ s/ //g;
		$t1 =~ tr/a-z/A-Z/;
...
sub Kasiski(@)
{
   ...
   $occur = 3 unless $occur;        # set default value if occur is not given
   ...
   for ( ; $min <= $max; $min++)
   {
      ...
      for( my $i=0, my $l=0; $i < length($buff)/2; $i++)
      {
         ...
         if ( $cnt >= $occur )
         {
            ...
            {
					my $dif = $pats[$k+1] - $pats[0];
					my @m = Factor( $dif);
               $pats_diff_f .=  "$dif(" . $m[0].");";

               push(@pats_diff , $dif);
            }
				$pats_diff_f =~ s/\^(\d+)/<sup>$1<\/sup>/g;
				$pats_diff_f =~ s/ //g;
            Log("Delta(x) b/w these patterns $pats_diff_f\n") if $opt_verb;
